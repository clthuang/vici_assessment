...
**Pivot from building to orchestrating**: Instead of implementing custom browser control tools from scratch, leverage existing battle-tested MCP servers (Playwright MCP, browser-use, Chrome DevTools) and focus on building a thin orchestration layer that:
...
**Go/No-Go**: If POC fails after 5 hours, evaluate whether to:
- Switch to official MCP SDK
- Try browser-use instead of Playwright MCP
- Revisit the approach entirely
