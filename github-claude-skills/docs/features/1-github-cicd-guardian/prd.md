...
|-----------|----------------------|------------|
| "Fix my CI" | Diagnose + propose fix in one response (read + analyze + propose are free) | "Here's the fix. Apply it?" (single confirmation before writing) |
| "My pipeline is broken" | Fetch logs + analyze + propose fix (read-only until write) | "Here's the root cause and proposed fix. Apply it?" |
...
- "Fix my CI" -> must diagnose and propose fix, but ask before writing/executing
